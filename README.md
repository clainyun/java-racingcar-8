...
- [ ] 경주할 자동차의 이름을 입력받는다 (예: pobi,woni,jun)
- [ ] 자동차 이름을 쉼표로 분리한다
...
### 4. 결과 출력
> 각 차수 실행 결과와 최종 우승자를 출력한다.
- [ ] 각 자동차의 실행 결과를 출력한다 (예: pobi : --)
- [ ] 자동차 경주 게임의 최종 우승자를 찾는다
- [ ] 우승자는 한 명 이상일 수 있다
- [ ] 여러 명일 경우 쉼표로 구분해 출력한다 (예: 최종 우승자 : pobi, jun)
...
- [ ] 3항 연산자는 사용하지 않는다
- [ ] 메서드는 한 가지 일만 수행하도록 구현한다 (SRP 준수)
...
- [ ] 모든 테스트(`./gradlew clean test`)가 성공적으로 통과한다
- [ ] 출력 형식이 예시와 정확히 일치한다
- [ ] 예외 발생 시 `IllegalArgumentException` 이후 종료된다
- [ ] README의 모든 체크박스 항목이 구현 완료되었다
- [ ] 기능 단위로 커밋이 분리되어 있다